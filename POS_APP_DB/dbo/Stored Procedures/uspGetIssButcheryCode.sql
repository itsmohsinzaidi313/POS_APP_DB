Create proc [dbo].[uspGetIssButcheryCode] 
@SId as int
as
...
set @TableName = 'IssuanceButcheryMaster';
set @VoucherPara= 'ISSB-';

SET @SQLString= 'select @NewVoucher = isnull(Max(IssBNo),0) from '+@TableName+' where SId=''' + CONVERT(VARCHAR(10),@SId, 101) + '''';
