CREATE PROCEDURE [dbo].[uspApiInsertFixedDeals]
	@orderKey varchar(100),
	@xml xml
AS
	print '---INSERT FIXED DEALS---'
	declare
		@counterId int,
		@tiltId int,
	...
	declare cFixedDeals cursor local for select 
		c.value('@Id','int'), 
	...
	open cFixedDeals
	fetch next from cFixedDeals into @dealId, @dealName, @dealQty, @dealPrice, @categoryName, @dealComment
	...
	begin
		...
		print 'inserting deal_item' + cast(@count as varchar(10))
		...
		''
		from @xml.nodes('/Order/FixedDeals/FixedDeal/Items/MenuItem') 
		as t(c)
