...
create Proc [dbo].[InsertProductionWareHouseBranch]
@Date as datetime,
@InvoiceId as int,
@IssId as int,
@TRInId as int,
...
SET @SQLString2='Insert Into '+@WareHouse+' (Date,ItemId,Unit,InvoiceId,IssId,TRInId,TROutId,InvAdjId,Qty,Rate,Type,BRId,SId,PDId,Did) 
SELECT 
...
myXML.value(''./@Qty'', ''decimal (18,2)''),myXML.value(''./@Rate'', ''decimal (18,2)''),''' + CONVERT(VARCHAR(10),@Type, 101) + ''',''' + CONVERT(VARCHAR(10),@BRId, 101) + ''',''' + CONVERT(VARCHAR(10),@SId, 101) + ''',''' + CONVERT(VARCHAR(10),@PDId, 101) + ''',''' + CONVERT(VARCHAR(10),@Did, 101) + '''
