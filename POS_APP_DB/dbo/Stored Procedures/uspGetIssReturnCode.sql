CREATE proc [dbo].[uspGetIssReturnCode] 
@BRId as int
as
...
set @TableName = 'IssuanceReturnMaster';
set @VoucherPara= 'ISSR-';

SET @SQLString= 'select @NewVoucher = isnull(Max(IssRNo),0) from '+@TableName+' where BRId =''' + CONVERT(VARCHAR(10),@BRId , 101) + '''';
