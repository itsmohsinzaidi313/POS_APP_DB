...
Create Proc [dbo].[ReportInvoiceWisePurchaseReturnReport]--'6/26/2013 12:00:00 AM','7/31/2013 12:00:00 AM','Admin','30'
@PINOId as nvarchar(50),
@Login as nvarchar(max)
as
Declare @ReportName as nvarchar(max);
set @ReportName='Purchase Return Invoice wise';
select
...
inner join Split(@PINOId,',') sp on sp.items =prm.InvoiceId
where  ipl.BRId=0
