...
CREATE Proc [dbo].[ReportCateWiseIssuanceReturn]--'6/27/2013 12:00:00 AM','7/30/2013 12:00:00 AM','Admin','30'
@From as datetime,
@To as Datetime,
...
set @ReportName='Branch Issuance Return Category WISE';
select CONVERT(VARCHAR(11),@From,106) as [From],CONVERT(VARCHAR(11),@To,106) as [To],@Login as LoginUser,ism.Date,ism.IssRNo,@ReportName as store,Un.Unit, b.branch,c.Category,sc.Subcategory,i.Item,
...
(Select Cast (isnull(Round([dbo].uspGetItemAvgRateFunc(s.SId,i.ItemId,@To,1),2),0)AS DECIMAL (18,2)) ) as UnitPrice,
...
inner join Split(@CategoryId,',') sp on sp.items = C.CId
