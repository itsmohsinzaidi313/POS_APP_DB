CREATE proc [dbo].[uspGetPoCompanyCode] 
@COId as int
as
...
set @TableName = 'PurchaseOrderMaster_Store';
set @VoucherPara= 'PO-';
SET @SQLString= 'select @NewVoucher = isnull(Max(PONo),0) from '+@TableName+' where COId=''' + CONVERT(VARCHAR(10),@COId, 101) + '''';
