CREATE proc [dbo].[uspGetPhysicalBranchCode] 
@BRId as int
as
...
set @TableName = 'PhysicalStockMaster_Branch';
set @VoucherPara= 'PHO-';
SET @SQLString= 'select @NewVoucher = isnull(Max(PSNO),0) from '+@TableName+' where BRId=''' + CONVERT(VARCHAR(10),@BRId, 101) + '''';
