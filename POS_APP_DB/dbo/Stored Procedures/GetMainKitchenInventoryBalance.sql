CREATE Proc [dbo].[GetMainKitchenInventoryBalance]--'62'
@BRId as int
as
...
( Select isnull(Sum(Qty),0) from WareHouse_Branch where [Type]='In' and ItemId=i.ItemId and BRId=@BRId)
 -
( Select isnull(Sum(Qty),0) from WareHouse_Branch where [Type]='Out' and ItemId=i.ItemId and BRId=@BRId))
...
where ipl.SId=0 and ipl.BRId=@BRId and i.[Type]='Non Butchery'and sc.SubCategory<>'Sub Recipe'
order by i.Item 
