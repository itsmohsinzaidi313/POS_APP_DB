...
CREATE Proc [dbo].[ReportCategoryWisePhysicalStockStore]--'6/26/2013 12:00:00 AM','6/27/2013 12:00:00 AM','Admin','28'
@From as datetime,
@To as Datetime,
...
set @ReportName='Physical Stock Store Category wise';
...
isnull
(
(Select isnull(sum(Qty),0) from WareHouse_Store w where [Type]='In' and w.ItemId=i.ItemId and w.SId=pm.SId and w.Date<=pm.Date )
-
(Select isnull(sum(Qty),0) from WareHouse_Store w where [Type]='Out' and w.ItemId=i.ItemId and w.SId=pm.SId and w.Date<=pm.Date)
,0)
as InventoryBalance,
