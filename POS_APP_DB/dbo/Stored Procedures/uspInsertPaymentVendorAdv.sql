create proc [dbo].[uspInsertPaymentVendorAdv]
@VN as nvarchar(50),
@Date datetime,
@SPId int,
@Amount decimal(18,2),
...
--insert into SupplierLedger(VoucherId,Amount,[Type],SPId,Date,COId,VoucherType,VN,InvoiceId)
--Select @BPId,
--          myXML.value('./@Amount', 'decimal(18,2)'),
...
if @PaymentMode = 'BANK'
Begin
--EXECUTE [uspInsertSupplierLedger] @BPId,@Amount,'D',@SPId,@Date,@COId,'BANK PAYMENT VOUCHER',@VN,0

insert into SupplierLedger(VoucherId,Amount,[Type],VId,Date,COId,VoucherType,VN,InvoiceId,IsAdvance)
values 
(@BPId,@Amount,'D',@SPId,@Date,@COId,'BANK PAYMENT VOUCHER',@VN,0,1)
...
Begin
--EXECUTE [uspInsertSupplierLedger] @CPId,@Amount,'D',@SPId,@Date,@COId,'CASH PAYMENT VOUCHER',@VN,0

insert into SupplierLedger(VoucherId,Amount,[Type],VId,Date,COId,VoucherType,VN,InvoiceId,IsAdvance)
values 
(@CPId,@Amount,'D',@SPId,@Date,@COId,'CASH PAYMENT VOUCHER',@VN,0,1)
