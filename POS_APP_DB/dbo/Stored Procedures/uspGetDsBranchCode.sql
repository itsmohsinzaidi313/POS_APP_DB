CREATE proc [dbo].[uspGetDsBranchCode]--25
@BRId as int
as
...
Declare @Kitchen varchar(50);

select @Kitchen = Branch from Branch where BRId = @BRId;

set @TableName = 'DemandSheetMaster_Branch';
set @VoucherPara= 'KDS-';
--SET @SQLString= 'select @NewVoucher = isnull(Max(DSNo),''0'') from '+@TableName+' 
--''where BRId=''' + CONVERT(VARCHAR(10),@BRId, 101) + '''
--';
