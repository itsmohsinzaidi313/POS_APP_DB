...
CREATE Proc [dbo].[ReportInventoryAdjusmentlStockStore]--'admin','32'
@Login as nvarchar(max),
@AdjNoId as varchar(50)
as
select @Login as LoginUser,idms.Date,idms.AdjNo,s.Store,c.Category,sc.Subcategory,i.Item,
...
isnull
(
(Select isnull(sum(Qty),0) from WareHouse_Store w where [Type]='In' and w.ItemId=i.ItemId and w.SId=idms.SId and Date <=idms.Date)
-
(Select isnull(sum(Qty),0) from WareHouse_Store w where [Type]='Out' and w.ItemId=i.ItemId and w.SId=idms.SId and Date <=idms.Date)
,0)
