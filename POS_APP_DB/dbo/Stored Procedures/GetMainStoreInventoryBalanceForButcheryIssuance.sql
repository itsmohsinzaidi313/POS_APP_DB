CREATE Proc [dbo].[GetMainStoreInventoryBalanceForButcheryIssuance]--85
@Sid as int
as
...
(Select isnull(Sum(Qty),0) from WareHouse_Store where [Type]='In' and ItemId=i.ItemId and SId=@Sid and  IssRTId>0 )
)
 -
...
Cast(isnull(Round(
--(select isnull(avg(Rate),0) from WareHouse_Store where ItemId=i.ItemId  and SId=@Sid and BUTId>0)/
--(select isnull(IssFactor,0) from ItemUnit where ItemId=i.ItemId),2),0) AS DECIMAL (18,2))as Rate,
(select isnull(avg(Rate),0) from ButcheryReturnDetail where ItemId=i.ItemId)/
(select isnull(IssFactor,0) from ItemUnit where ItemId=i.ItemId),2),0) AS DECIMAL (18,2))as Rate,
