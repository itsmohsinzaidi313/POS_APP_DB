...
CREATE Proc [dbo].[ReportDateWiseDemandSheetStore]
@COId as int,
@From as datetime,
@To as Datetime,
...
set @ReportName='Demand Sheet Store Date Wise'
select CONVERT(VARCHAR(11),@From,106) as [From],CONVERT(VARCHAR(11),@To,106) as [To],@Login as LoginUser,dm.Date,dm.DSNo,@ReportName as Store,c.Category,sc.Subcategory,i.Item,
...
isnull
(
(Select isnull(sum(Qty),0) from WareHouse_Store w where [Type]='In' and w.ItemId=i.ItemId and w.SId=dm.SId and w.Date<dm.Date and w.OpenInvid = 0)
-
(Select isnull(sum(Qty),0) from WareHouse_Store w where [Type]='Out' and w.ItemId=i.ItemId and w.SId=dm.SId and w.Date<dm.Date and w.OpenInvid = 0)
,0)+
...
where dm.Date between @From and @To and dm.COId=@COId and
dm.SId=@SId
