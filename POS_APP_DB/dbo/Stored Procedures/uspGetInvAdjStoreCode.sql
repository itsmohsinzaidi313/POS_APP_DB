CREATE proc [dbo].[uspGetInvAdjStoreCode] 
@SId as int
as
...
set @TableName = 'InvAdjMaster_Store';
set @VoucherPara= 'ADJ-';

SET @SQLString= 'select @NewVoucher = isnull(Max(AdjNo),0) from '+@TableName+' where SId=''' + CONVERT(VARCHAR(10),@SId, 101) + ''' ';
