...
CREATE proc [dbo].[UspGetSubCateItemLedgerTest]--'7/01/2013 12:00:00 AM','7/10/2013 12:00:00 AM','14'
@From as datetime,
@To as Datetime,
...
select CONVERT(VARCHAR(11),@From,106) as [From],CONVERT(VARCHAR(11),@To,106) as [To],sb.SubCategory as Category,i.ItemCode as SubCategory,
...
inner join Split(@SubCategoryId,',') sp on sp.items = sb.SBId 
inner join Unit U on iu.PurUnit=U.UId
where 
--ws.PDId = 0 and 
ws.Date between @From and @To and ws.SId = @SId and (ws.InvAdjId = @Adj OR @Adj is null)
group by i.ItemId,i.Item,ws.Date,ws.[Type],ws.[Desc],ws.Id,sb.SubCategory,U.Unit,iu.IssFactor,
ws.Unit,i.ItemCode,iu.IssUnit
