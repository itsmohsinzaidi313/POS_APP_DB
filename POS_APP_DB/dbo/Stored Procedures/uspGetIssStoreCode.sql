CREATE proc [dbo].[uspGetIssStoreCode] 
@SId as int
as
...
set @TableName = 'IssuanceMaster_Store';
set @VoucherPara= 'ISS-';

SET @SQLString= 'select @NewVoucher = isnull(Max(IssNo),0) from '+@TableName+' where SId=''' + CONVERT(VARCHAR(10),@SId, 101) + '''';
