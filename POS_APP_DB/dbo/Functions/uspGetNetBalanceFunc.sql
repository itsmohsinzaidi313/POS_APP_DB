create function [dbo].[uspGetNetBalanceFunc]
(
@AccountNo  int,
@COId  int
)
...
select @Debit = isnull(sum(Amount),0) from GL where [Type] = 'D' and CAId = @AccountNo and COId = @COId and APId = @FiscalId
select @Credit = isnull(sum(Amount),0) from GL where [Type] = 'C' and CAId = @AccountNo and COId = @COId and APId = @FiscalId
