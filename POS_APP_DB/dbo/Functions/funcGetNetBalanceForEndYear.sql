CREATE function [dbo].[funcGetNetBalanceForEndYear]
(
@AccountNo  int,
@COId  int
)
...
select @Debit = isnull(sum(Amount),0) from GL where [Type] = 'D' and CAId = @AccountNo and COId = @COId
select @Credit = isnull(sum(Amount),0) from GL where [Type] = 'C' and CAId = @AccountNo and COId = @COId
if (@AccountType = 'ASSETS') begin 
set @NetBalance = @Debit - @Credit 
set @FinalNetBalance = @NetBalance + @OpenBalance
end 
else if (@AccountType = 'LIABILITIES') begin 
set @NetBalance = @Credit - @Debit 
set @FinalNetBalance = @NetBalance + @OpenBalance
end 
