CREATE function [dbo].[uspGetNetBalanceFuncByFromToDate]
(
@DateFrom  datetime,
@DateTo  datetime,
@AccountNo  int,
...
Declare @AccPeriodDateFrom datetime;

set @FinalNetBalance =0;

select @FiscalId = max(APId),@AccPeriodDateFrom = [From] from AccountPeriod where IsActive = 1 and COId = @COId group by [From]
...
select @Debit = isnull(sum(Amount),0) from GL where [Type] = 'D' and CAId = @AccountNo and COId = @COId and Date between @DateFrom and @DateTo
select @Credit = isnull(sum(Amount),0) from GL where [Type] = 'C' and CAId = @AccountNo and COId = @COId and Date between @DateFrom and @DateTo
...
set @FinalNetBalance = @NetBalance ;

if @AccPeriodDateFrom > @DateFrom
Begin
